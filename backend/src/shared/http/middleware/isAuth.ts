...
const cache: Map<string, {
  // Payload cru do SSO externo (`response.json()`, fora do nosso controle de
  // schema); só `email`/`blocked` são consumidos. `any` confinado ao boundary.
  user: any,
  timestamp: number
}> = new Map();
...
export async function fetchUserData(userId: string) {
  ...
  const response = await fetch(appConfig.auth.checkAuthEndpoint, {
    headers: {
      'Cookie': `user=${userId}`,
